  ...
  async getwarning() {
    ...
    if (location_active1) {
      if (!link || !apikey[0] || !lat[0] || !lon[0]) {
        this.log.error(
          "Please check your config. Linkdata, API-key and location missing for location 1"
        );
      } else {
        urls[0] =
          link + "?key=" + apikey[0] + "&lat=" + lat[0] + "&lng=" + lon[0];
      }
    ...
    if (location_active2) {
      if (!link || !apikey[0] || !lat[0] || !lon[0]) {
        this.log.error(
          "Please check your config. Linkdata, API-key and location missing for location 2"
        );
      } else {
        urls[1] =
          link + "?key=" + apikey[1] + "&lat=" + lat[1] + "&lng=" + lon[1];
      }
    ...
    if (location_active3) {
      if (!link || !apikey[0] || !lat[0] || !lon[0]) {
        this.log.error(
          "Please check your config. Linkdata, API-key and location missing for location 3"
        );
      } else {
        urls[2] =
          link + "?key=" + apikey[2] + "&lat=" + lat[2] + "&lng=" + lon[2];
      }
    ...
    if (location_active4) {
      if (!link || !apikey[0] || !lat[0] || !lon[0]) {
        this.log.error(
          "Please check your config. Linkdata, API-key and location missing for location 4"
        );
      } else {
        urls[3] =
          link + "?key=" + apikey[3] + "&lat=" + lat[3] + "&lng=" + lon[3];
      }
    ...
    for (let url_read_index = 0; url_read_index < 5; url_read_index++) {
      ...
      if (location_active[url_read_index] && urls[url_read_index]) {
        this.log.debug(
          "location_active[url_read_index]: " + location_active[url_read_index]
        );
        try {
          ...
          this.log.debug(
            "location_active:" +
              url_read_index +
              " : " +
              location_active[url_read_index]
          );

          await this.setStateAsync(url_read_index + ".code", {
            val: data_code,
            ack: true,
          });
          ...
          if (data_code === 200 && location_active[url_read_index]) {
            ...
            await this.setStateAsync(url_read_index + ".requestDate", {
              val: data_requestdate,
              ack: true,
            });
            await this.setStateAsync(url_read_index + ".forecastId", {
              val: data_forecastid,
              ack: true,
            });
            await this.setStateAsync(url_read_index + ".forecastText", {
            ...
            await this.setStateAsync(url_read_index + ".forecastDate", {
            ...
            await this.setStateAsync(url_read_index + ".callsLeft", {
              val: data_callsLeft,
              ack: true,
            });
            ...
            if (
              mail_active[url_read_index] &&
              (data_forecasttext === "Eis!" || test_massage)
            ) {
            ...
            if (
              pushover_active[url_read_index] &&
              (data_forecasttext === "Eis!" || test_massage)
            ) {
              this.log.debug(
                "pushover_message:" + pushover_message[url_read_index]
              );
              /*
...
*/
            ...
            if (
              telegram_active[url_read_index] &&
              (data_forecasttext === "Eis!" || test_massage)
            ) {
    ...
    for (let create_index = 0; create_index < 5; create_index++) {
      if (location_active[create_index]) {
        ...
        await this.setObjectNotExistsAsync(
          create_index + ".callsResetInSeconds",
          {
            type: "state",
            common: {
